...
### 2026.07.23 - GRID
- 그리드에서 BFS로 최단거리 구하는 것까지는 어찌어찌 이해했는데, 도착까지 실제 "경로"가 뭐였는지 구하는 건 또 다른 문제였다. 처음엔 이걸 어떻게 저장해야 하나 감이 안 왔다.
- 알고 보니 답은 `parent` 배열 하나였다. 새 칸을 방문할 때 "나 지금 어디서 왔어"만 같이 적어두면, 나중에 도착점부터 거꾸로 따라 올라가면서 경로 전체를 만들 수 있다는 걸 알았다. `visited` 채우던 자리에 한 줄만 추가하면 되는 거라 막상 알고 나니 별거 아니었다.
- 좌표를 `r * m + c`로 인코딩해서 배열 하나에 저장하는 방식은 처음엔 이게 뭔 소린가 싶었는데, `(row, col)` 두 숫자를 하나로 합쳐놓은 거라는 걸 이해하고 나니 오히려 코드가 더 단순해 보였다.
- bfs 코드를 바탕으로 dfs를 사용하여 경로 도착여부 확인 코드를 작성했을땐, true/false로 좀더 간결하게 코드짜는것이 가능했다.

### 2026.07.26 - GRID(2)
- 아직 grid에 대해서 미흡한거같아서 좀더 코드를 직접 작성하여 숙련도를 익히는데 도움이 되었다.
- 결국 dfs나 bfs나 방법만 다를뿐 서로 필요에 따라 선택하는 방법을 익혔다.